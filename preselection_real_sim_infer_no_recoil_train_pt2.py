...
outputDir   = "/eos/user/f/fdmartin/FCC365_MVA_train_realsim/preselection_total_with_inference_no_recoil_train_pt2"
...
###############################################################################################################################################
      # Eliminate particles that correspond to muons y electrons
        ...
        if doInference:
            tmva_helper = TMVAHelperXGB("/eos/user/f/fdmartin/FCC365_MVA_train_realsim/train/bdt_model_example.root", "bdt_model") # read the XGBoost training
    ...
    def output():
        branchList = ["jj_m", "hbb_m", "higgs_recoil_m", "cosTheta_miss", "missingEnergy_energy_fixed", "missing_p_fixed", "hbb_p"]
        if doInference:
