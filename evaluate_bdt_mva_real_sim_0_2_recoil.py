...
parser.add_argument("-i", "--input", type=str, default="/eos/user/f/fdmartin/FCC365_MVA_train_realsim_recoil/train/bdt_model_example.pkl", help="Input pkl file")
parser.add_argument("-o", "--outDir", type=str, default="/eos/user/f/fdmartin/FCC365_MVA_train_realsim_recoil/evaluate_plots_0_2", help="Output directory")
...
def plot_score_threshold():
    ...
    plt.figure(figsize=(8, 6))
    plt.hist(train_predictions[(train_labels == 1) & (train_predictions > 0.2)], bins=50, range=(0.2, 1),
             histtype='step', label='Training Signal (>0.2)', color='blue', density=True)
    plt.hist(train_predictions[(train_labels == 0) & (train_predictions > 0.2)], bins=50, range=(0.2, 1),
             histtype='step', label='Training Background (>0.2)', color='red', density=True)
    plt.hist(test_predictions[(test_labels == 1) & (test_predictions > 0.2)], bins=50, range=(0.2, 1),
             histtype='step', label='Testing Signal (>0.2)', color='blue', linestyle='dashed', density=True)
    plt.hist(test_predictions[(test_labels == 0) & (test_predictions > 0.2)], bins=50, range=(0.2, 1),
             histtype='step', label='Testing Background (>0.2)', color='red', linestyle='dashed', density=True)
    plt.xlabel('BDT Score')
    plt.ylabel('Number of Events (normalized)')
    plt.title('BDT Score Distribution (Score > 0.2)')
    plt.legend()
    plt.grid()
...
def plot_kinematics_threshold():
    ...
    for var in kinematics:
        if var in variables:
            ...
            train_signal = train_data[(train_scores > 0.2) & (train_labels == 1), idx]
            train_background = train_data[(train_scores > 0.2) & (train_labels == 0), idx]
            test_signal = test_data[(test_scores > 0.2) & (test_labels == 1), idx]
            test_background = test_data[(test_scores > 0.2) & (test_labels == 0), idx]

            plt.figure(figsize=(8, 6))
            plt.hist(train_signal, bins=50, histtype='step', label='Training Signal (>0.2)', color='blue', density=True)
            plt.hist(train_background, bins=50, histtype='step', label='Training Background (>0.2)', color='red', density=True)
            plt.hist(test_signal, bins=50, histtype='step', label='Testing Signal (>0.2)', color='blue', linestyle='dashed', density=True)
            plt.hist(test_background, bins=50, histtype='step', label='Testing Background (>0.2)', color='red', linestyle='dashed', density=True)
            plt.xlabel(var)
            plt.ylabel('Number of Events (normalized)')
            plt.title(f'Histogram of {var} (BDT Score > 0.2)')
            plt.legend()
            plt.grid()
